...
fn draw_hierarchy_node(ui: &Ui, graph: &Vec<SavedNode>, node_idx: usize, selected_node_idx: &mut Option<NonZeroUsize>) {
    if let Some(node) = graph.get(node_idx) {
        ...
        if node_idx != 0 {
            ...
            flags.set(TreeNodeFlags::SELECTED, matches!(selected_node_idx, Some(x) if usize::from(*x) == node_idx));
